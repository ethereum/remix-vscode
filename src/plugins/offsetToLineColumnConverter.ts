...
export default class OffsetToLineColumnConverter extends Plugin {
  ...
  offsetToLineColumn(rawLocation, file, sources, asts) {
    if (!this.lineBreakPositionsByContent[file]) {
      ...
      if (!asts || (file === 0 && sourcesArray.length === 1)) {
        ...
        this.lineBreakPositionsByContent[
          file
        ] = this.sourceMappingDecoder.getLinebreakPositions(
          sources[sourcesArray[0]].content
        );
      } else {
    ...
    return this.sourceMappingDecoder.convertOffsetToLineColumn(
      rawLocation,
  ...
  offsetToLineColumnWithContent(rawLocation, file, content) {
    this.lineBreakPositionsByContent[
      file
    ] = this.sourceMappingDecoder.getLinebreakPositions(content);
    return this.sourceMappingDecoder.convertOffsetToLineColumn(
      rawLocation,
      this.lineBreakPositionsByContent[file]
    );
