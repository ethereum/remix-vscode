...
class CompilerImports extends engine_1.Plugin {
    ...
    importExternal(url, targetPath, cb) {
        ...
        (loadingMsg) => { this.emit('message', loadingMsg); }, (error, content, cleanUrl, type, url) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (error)
                return cb(error);
            try {
    ...
    resolveAndSave(url, targetPath) {
        return new Promise((resolve, reject) => {
            if (url.indexOf('remix_tests.sol') !== -1)
                resolve(remixTests.assertLibCode);
                            ...
                            if (localhostProvider.isConnected()) {
                                ...
                                return async.tryEach([
                                    ...
                                    (cb) => { if (!splitted) {
                                        ...
                                        this.resolveAndSave('localhost/installed_contracts/' + splitted[1] + '/contracts/' + splitted[2], null).then((result) => cb(null, result)).catch((error) => cb(error.message));
                                    } },
                                    ...
                                    (cb) => { if (!splitted) {
                                        ...
                                        this.resolveAndSave('localhost/node_modules/' + splitted[1] + '/contracts/' + splitted[2], null).then((result) => cb(null, result)).catch((error) => cb(error.message));
                                    } }
                                ], (error, result) => {
                                    if (error) {
